...
class ChatAPIClient:
    def __init__(self, root):
        self.root = root
        self.root.title("Chat API Client# PUK PUKUMPEE")
        ...
        self.token = None
        self.username = None  # เพิ่มเพื่อเก็บ username หลังล็อกอิน
    ...
    def create_widgets(self):
        ...
        status_frame = ttk.LabelFrame(connection_tab, text="API Status :: Find API--> http://pukumpee.lovestoblog.com")
    ...
    def login(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.username = username  # เก็บ username หลังล็อกอินสำเร็จ
    ...
    def refresh_rooms(self):
        ...
        try:
            ...
            response = requests.get(f"{self.base_url}/chat/rooms", headers=headers)
            if response.status_code == 200:
                rooms = response.json()
    ...
    def on_room_select(self, event):
        ...
        if selection:
            ...
            self.load_chat_history()  # เรียกฟังก์ชันเพื่อโหลดประวัติ
            
    def load_chat_history(self):
        if not self.selected_room or not self.token:
            return
        
        try:
            headers = {"Authorization": f"Bearer {self.token}"}
            response = requests.get(f"{self.base_url}/chat/rooms/{self.selected_room}/messages", headers=headers)
            if response.status_code == 200:
                messages = response.json()
                self.chat_display.config(state="normal")
                self.chat_display.delete(1.0, tk.END)
                for msg in messages:
                    # แสดงประวัติแชทพร้อม timestamp และ user_id
                    timestamp = msg.get("created_at", "Unknown time")
                    user_id = msg.get("user_id", "Unknown user")
                    content = msg.get("content", "")
                    display_text = f"[{timestamp}] User {user_id}: {content}\n"
                    self.chat_display.insert(tk.END, display_text)
                self.chat_display.config(state="disabled")
            else:
                messagebox.showerror("Error", f"Failed to load chat history! Status: {response.status_code}")
        except requests.exceptions.RequestException as e:
            messagebox.showerror("Error", f"Error loading chat history: {str(e)}")
    ...
    def send_message(self):
        ...
        if message:
            try:
                ...
                if response.status_code == 200:
                    self.chat_display.config(state="normal")
                    # แสดงข้อความที่ส่งพร้อม timestamp ปัจจุบัน
                    timestamp = datetime.now().isoformat()
                    self.chat_display.insert("end", f"[{timestamp}] {self.username}: {message}\n")
                    self.chat_display.config(state="disabled")
